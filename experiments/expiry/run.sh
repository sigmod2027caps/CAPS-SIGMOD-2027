...
echo "=== $DATAFLOW expiry (online index, 7-day horizon) ==="
...
echo -e "\n[run: dataflow + online expiry]"
rm -f "$EXPDIR"/{expiry_memory.csv,expiry_stats.csv}
"$FLINK" run --parallelism 1 --class NexmarkMonitorNewUsersCaps \
  ...
  unused \
  expiry "$EXPDIR" "$HORIZON_MS" "$QUERY_LEN_MS" > "$EXPDIR/flink.log" 2>&1

echo "expiry_stats.csv:"
cat "$EXPDIR/expiry_stats.csv"
