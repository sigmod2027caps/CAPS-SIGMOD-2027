...
public class GenealogFileSink<T extends GenealogTuple> extends RichSinkFunction<T> {
  ...
  private transient long totalTraversalNanos;
  ...
  public void invoke(T tuple, Context context) {
    ...
    totalTraversalNanos += System.nanoTime() - tStart;
  ...
  public void close() throws Exception {
    ...
    pw.flush();
    pw.close();
    String memFile = settings.memoryFile();
    if (memFile != null) {
      String traversalFile = memFile.replace("memory_", "traversal_");
      if (traversalFile.equals(memFile)) {
        traversalFile = memFile + ".traversal";
      }
      try (PrintWriter tw = new PrintWriter(new FileWriter(traversalFile))) {
        tw.println(totalTraversalNanos);
      }
    }
    super.close();
  }
