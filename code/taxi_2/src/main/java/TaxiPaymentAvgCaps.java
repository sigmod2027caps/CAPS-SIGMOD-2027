 ...
 * {@link HowMuch} meta-data with one counter per virtual source: SC counts
...
public class TaxiPaymentAvgCaps {
  ...
  public static final int SC = 0;  // card rides (CRD branch)
  public static final int SK = 1;  // cash rides (CSH branch)
  ...
  public static void main(String[] args) throws Exception {
    ...
    IndexingSink.HmFn<ZoneAvg> hm =
        (IndexingSink.HmFn<ZoneAvg>) z -> z.hm;

    // Card virtual source (SC): average tip per zone. 1h/10min slide matches noprov taxi_2.
    DataStream<ZoneAvg> cardMapped = sourceStream
        .filter(r -> "CRD".equals(r.payment))
        .name("FILTER-CARD")
        .map(new ToZoneValue(true))
        .name("MAP-CARD");
    cardMapped = CapsMetadataProbe.attach(cardMapped, memoryVolume, "MAP-CARD", hm);
    DataStream<ZoneAvg> card = cardMapped
        .keyBy(z -> z.zone)
        .window(SlidingEventTimeWindows.of(Time.hours(1), Time.minutes(10)))
        .aggregate(new AvgAggregate())
        .name("AGG-CARD");
    card = CapsMetadataProbe.attach(card, memoryVolume, "AGG-CARD", hm);
    ...
    DataStream<ZoneAvg> cashMapped = sourceStream
        ...
        .name("MAP-CASH");
    cashMapped = CapsMetadataProbe.attach(cashMapped, memoryVolume, "MAP-CASH", hm);
    DataStream<ZoneAvg> cash = cashMapped
        .keyBy(z -> z.zone)
        .window(SlidingEventTimeWindows.of(Time.hours(1), Time.minutes(10)))
        .aggregate(new AvgAggregate())
        .name("AGG-CASH");
    cash = CapsMetadataProbe.attach(cash, memoryVolume, "AGG-CASH", hm);
    ...
    if (memoryVolume) {
      joined
          .addSink(new CapsCardinalityDiscardSink<ZoneAvg>())
          .name("SINK");
    } else if (benchMode) {
      // Bench sink: same sample coin as noprov, plus SC/SK how-much index.
      joined
          .addSink(BenchSinks.create(args, 2,
              (IndexingSink.TsFn<ZoneAvg>) z -> z.timestamp,
              (IndexingSink.HmFn<ZoneAvg>) z -> z.hm,
    ...
    CapsMetadataProbe.writeIfEnabled(env.execute("TaxiPaymentAvg-HowMuch"),
        args, 2, "taxi_2");
  }
  ...
  public static class ToZoneValue implements MapFunction<Ride, ZoneAvg> {
    // Virtual-source provenance tag: SC on card rides, SK on cash.
  ...
  public static class AvgAggregate
    ...
    public ZoneAvg getResult(Acc acc) {
      ...
      return new ZoneAvg(acc.zone, acc.maxTs, avg, 0, acc.hm.copy());
    }
