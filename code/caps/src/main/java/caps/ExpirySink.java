...
// Paper 5.2: two CapsIndexes side-by-side — full (ground truth) and expiring on a 6h schedule.
// In-horizon queries must match exactly; same reorder buffer as IndexingSink.
public class ExpirySink<T> extends RichSinkFunction<T> {

  private static final long serialVersionUID = 1L;

  /** Sample (and expire) every 6 hours of event time. */
  ...
  private void release(Entry e) {
    // Both indexes see the same time-ordered stream; only expiring gets expireBefore.
  ...
  public void close() throws Exception {
    ...
    // Queries fully inside the retained horizon must be exact despite expiry.
    // Fixed window length (ten slides of the dataflow's window, same as the
    // query-latency experiment), uniformly random start inside the horizon.
    ...
    int mismatches = 0;
    for (int i = 0; i < CHECK_QUERIES; i++) { // verification + JIT warm-up
    ...
    for (int i = 0; i < CHECK_QUERIES; i++) { // timed pass, per index
