...
// Offline Section-5 tool: load a sink CSV into CapsIndex, run query/delta/bench/selftest.
// Last channels columns are HowMuch counters; tsCol defaults to 1.
public final class Main {
  ...
  static List<Record> load(String file, int tsCol, int channels) throws Exception {
    ...
    // Sink outputs are only per-key time-ordered; the index needs a globally
    // time-ordered append sequence.
    records.sort(Comparator.comparingLong(r -> r.ts));
    return records;
  }

  /**
   * Query-latency experiment (paper 5.1): the same random time-range queries
   * answered by the array-scan baseline and by the prefix-sum index.
   * Prints CSV to stdout: method,records,queries,avg_us,total_ms.
   */
  private static void bench(List<Record> records, int channels, int numQueries) {
    ...
    // Random endpoints, not fixed-length windows — unlike the Flink benchmark sinks.
    ...
    // Correctness cross-check plus JIT warmup.
  ...
  private static void selfTest() {
    paperPrefixSumExample();
    paperSummarizeExample();
    expiryTest();
    versioningTest();
    sparseIndexTest();
    ringExpiryTest();
  ...
  /** Paper Fig. prefixsum: query [3,5] must give {(S1,7),(S2,5)}. */
  private static void paperPrefixSumExample() {
    CapsIndex index = paperIndex();
    double[] hp = index.howMuch(3, 5);
    check("prefixsum S1", 7, hp[0]);
    check("prefixsum S2", 5, hp[1]);
  }

  /** Paper Fig. prefixC: after summarizing [1,3], query [3,5] estimates {(S1,7),(S2,6.33)}. */
  private static void paperSummarizeExample() {
    CapsIndex index = paperIndex();
  ...
  /** Paper 5.2: after expiring records 1-3, queries with ts >= 4 stay exact. */
  private static void expiryTest() {
    ...
    check("expiry S1", 8, hp[0]);   // 3+3+2
    check("expiry S2", 4, hp[1]);   // 3+0+1
    boolean threw = false;
    try {
      index.howMuch(2, 5);
  ...
  /** Paper Fig. versioning: S1 2->6 (dHP=+4, dRho=+0.25), S2 2->2 (0, -0.25). */
  private static void versioningTest() {
    CapsIndex index = new CapsIndex(2);
    index.append(1, new long[] {2, 2});   // window [1,2]: S1=2, S2=2
    index.append(3, new long[] {6, 2});   // window [3,4]: S1=6, S2=2
  ...
  /** Sparse index correctness across level boundaries (n >> CAPACITY^2). */
  private static void sparseIndexTest() {
    ...
    long t = 0;
    for (int i = 0; i < n; i++) {
      t += rnd.nextInt(3); // duplicates and gaps
  ...
  /**
   * Ring buffer + B-tree front-deletion at scale: interleave expiry rounds
   * with fresh appends (ring slots get reused, separator levels shed their
   * fronts) and cross-check in-horizon queries against the full array store.
   */
  ...
  private static CapsIndex paperIndex() {
    // Consistent with paper Figs. prefixsum/prefixC: prefix sums at record 2
    // are (S1=2, S2=8), at record 5 (S1=9, S2=13); bucket [1,3] totals (3, 10).
    // Original per-record meta-data, timestamps 1..6:
    // S1: 1,1,1,3,3,2   S2: 4,4,2,3,0,1
    CapsIndex index = new CapsIndex(2);
